...
echo %date% ^|^|%time% : gConsolePassX 1.0 was started.>>AppsDataBase\log.txt
...
if not exist AppsDataBase\m.exe ( set errorcode=7 && goto check_error )
echo. Loading configuration ...
if not exist AppsDataBase\conf.ini (
   echo. Generating file "conf.ini" ...
   set Update=1
   goto generate_cgf
) else (
...
if %errorcode% == 1 (
	...
	echo. Please reinstall gConsolePassX on the same folder to fix this error.
	echo. gConsolePassX cannot run without [7z.exe], press any key to exit app.
	pause >nul && exit
) else if %errorcode% == 2 (
	...
	echo. Please reinstall gConsolePassX on the same folder to fix this error.
	echo. gConsolePassX cannot run without [Te.exe], press any key to exit app.
	pause >nul && exit
) else if errorlevel == 3 (
	echo %date% ^|^|%time% : Error, Incorrect State value found in conf.ini.>>AppsDataBase\log.txt
	cls
	echo ==============================================================================
	echo.                   gConsolePassX -Console Password Manager.
	echo ==============================================================================
	...
	echo. gConsolePassX 1.0 : Copyright 2021 - 2022 Abdullah Al Noman.
	echo.
	echo. There is a mistake in configuration under [\gConsolePassX\State\].
	echo. "State" value is invalid, you can manually set it to 1 or 2.
	echo. Else gConsolePassX will automatically try to fix this error.
	...
	echo. Step 01 : Install gConsolePassX 1.0 in another folder.
	...
	echo. Step 04 : Copy file and paste it in old gConsolePassX's AppsDataBase folder.
	echo. Step 05 : Run old gConsolePassX app and you should be able to proceed.
   ...
   echo %date% ^|^|%time% : Error, m.exe  missing from AppsDataBase folder.>>AppsDataBase\log.txt
	cls
	echo ==============================================================================
	echo.                   gConsolePassX -Console Password Manager.
	echo ==============================================================================
	...
	echo. gConsolePassX 1.0 : Copyright 2021 - 2022 Abdullah Al Noman.
	echo.
	echo. Error found, error code is 0x007
	echo. File missing from AppsDataBase folder [File name = m.exe].
	echo. Please reinstall gConsolePassX on the same folder to fix this error.
	echo. gConsolePassX cannot run without [m.exe], press any key to exit app.
	pause >nul && exit
)
...
@REM generate conf.ini and change setting automatically if needed
:generate_cgf
if exist AppsDataBase\71a.op ( set state=2 )
if not exist AppsDataBase\71a.op ( set state=1 )
echo #=Press [Ctrl + X] then [N] to go back to gConsolePassX from setting.  >AppsDataBase\conf.ini
echo #=Press [Ctrl + X] then [Y] then [Enter] to save changes.              >>AppsDataBase\conf.ini
echo #=Some changes only applies when you run gConsolePassX next time.      >>AppsDataBase\conf.ini
echo #=Change "Restore" value to 1 if you want to restore default settings. >>AppsDataBase\conf.ini
echo #=Don't forget to change "Update" value at the end.                    >>AppsDataBase\conf.ini
echo.>>AppsDataBase\conf.ini
echo #=[\gConsolePassX\Sys\]                                                >>AppsDataBase\conf.ini
echo #=Valid value for "State" is 1 and 2, don't change manually.           >>AppsDataBase\conf.ini
echo State=^%state%>>AppsDataBase\conf.ini
echo.>>AppsDataBase\conf.ini
echo #=AutoBackup will make automatic backup of database files.             >>AppsDataBase\conf.ini
echo #=Set value to 0 if you want to turn off, valid value [0 and 1].       >>AppsDataBase\conf.ini
echo AutoBackup=^%AutoBackup%>>AppsDataBase\conf.ini
echo.>>AppsDataBase\conf.ini
echo #=[\Window\Configuration\]                                             >>AppsDataBase\conf.ini
echo #=Do not change ScreenBufferSize value for maximum password limit.     >>AppsDataBase\conf.ini
echo ScreenBufferSize=^%ScreenBufferSize%>>AppsDataBase\conf.ini
echo.>>AppsDataBase\conf.ini
echo #=CursorColor can be any RGB color code to decimal converted value.    >>AppsDataBase\conf.ini
echo #=Red[255], Green[65280], Blue[16711680], Yellow[65535], Aqua[16776960]>>AppsDataBase\conf.ini
echo #=White[16777215], Black[0], Orange{42495], Brown[7482] etc.           >>AppsDataBase\conf.ini
echo CursorColor=^%CursorColor%>>AppsDataBase\conf.ini
echo.>>AppsDataBase\conf.ini
echo #=WindowSize has many options, better not to change for best size.     >>AppsDataBase\conf.ini
echo #=Instead you can change "WindowMode" to see which one you like.       >>AppsDataBase\conf.ini
echo WindowSize=^%WindowSize%>>AppsDataBase\conf.ini
echo.>>AppsDataBase\conf.ini
echo #=FontWeight can be minimum 256 to maximum 2304 [text thickness].      >>AppsDataBase\conf.ini
echo FontWeight=^%FontWeight%>>AppsDataBase\conf.ini
echo.>>AppsDataBase\conf.ini
echo #=WindowAlpha can be 170, 187, 204, 221, 238 and 255. [transparency]   >>AppsDataBase\conf.ini
echo WindowAlpha=^%WindowAlpha%>>AppsDataBase\conf.ini
echo.>>AppsDataBase\conf.ini
echo #=CursorSize can be minimum 25 to maximum 100. [cursor height]         >>AppsDataBase\conf.ini
echo #=This doesn't work all "CursorType", doesn't work with default[2]     >>AppsDataBase\conf.ini
echo CursorSize=^%CursorSize%>>AppsDataBase\conf.ini
echo.>>AppsDataBase\conf.ini
echo #=CursorType can be minimum 0 to maximum 5. [cursors design]           >>AppsDataBase\conf.ini
echo CursorType=^%CursorType%>>AppsDataBase\conf.ini
echo.>>AppsDataBase\conf.ini
echo #=bg_color can range from 1-7 and A-F. [window background]             >>AppsDataBase\conf.ini
echo bg_color=^%bg_color%>>AppsDataBase\conf.ini
echo.>>AppsDataBase\conf.ini
echo #=fg_color can range from 1-7 and A-F. [Window text color]             >>AppsDataBase\conf.ini
echo #=bg_color and fg_color cannot have same value.                        >>AppsDataBase\conf.ini
echo fg_color=^%fg_color%>>AppsDataBase\conf.ini
echo.>>AppsDataBase\conf.ini
echo #=WindowMode can have value 1-3. [Window mode/Window size]             >>AppsDataBase\conf.ini
echo #=Value 1 and 3 will disable "WindowSize" and "ScreenBufferSize"       >>AppsDataBase\conf.ini
echo WindowMode=^%WindowMode%>>AppsDataBase\conf.ini
echo.>>AppsDataBase\conf.ini
echo #=WinDefault value must be 0 for all settings to work.                 >>AppsDataBase\conf.ini
echo #=WinDefault value can be 0 or 1. [1 will disable other settings]      >>AppsDataBase\conf.ini
echo WinDefault=^%WinDefault%>>AppsDataBase\conf.ini
echo.>>AppsDataBase\conf.ini
echo #=[\Files\Name\]>>AppsDataBase\conf.ini
echo #=fname means file name, it can be anything you want.                  >>AppsDataBase\conf.ini
echo fname=%fname%>>AppsDataBase\conf.ini
echo.>>AppsDataBase\conf.ini
echo #=endswith value can only be [cmd] or [bat], else file will not run.   >>AppsDataBase\conf.ini
echo endswith=%endswith%>>AppsDataBase\conf.ini
echo.>>AppsDataBase\conf.ini
echo #=[\Support\RunApp]>>AppsDataBase\conf.ini
echo #=RunApp value must be higher then 1 for app to run.                   >>AppsDataBase\conf.ini
echo RunApp=^%RunApp%>>AppsDataBase\conf.ini
echo.>>AppsDataBase\conf.ini
echo #=[Update\Settings]>>AppsDataBase\conf.ini
echo #=Set "Restore" value to 1 if you want to restore default settings.    >>AppsDataBase\conf.ini
echo Restore=^%Restore%>>AppsDataBase\conf.ini
echo.>>AppsDataBase\conf.ini
echo #=Set Update value to 1 everytime you change any settings.             >>AppsDataBase\conf.ini
echo #=Else setting will not be updated, you can come back if you forget.   >>AppsDataBase\conf.ini
echo Update=^%Update%>>AppsDataBase\conf.ini
goto check_configuration
...
@REM load configuration from AppsDataBase\conf.ini
:load_configuration
set filename=%~nx0 && set confname=%fname%.%endswith%
set appname=%confname: =%
if %filename% NEQ %appname% goto change_filename
set string=%bg_color%%fg_color%
set color=%string: =%
@color %color%
set menuhover=%bg_color%%fg_color%%fg_color%%bg_color%
set menuhover=%menuhover: =%
if %Update% GTR 0 goto update_regestry
if %Update% LSS 1 goto check_user_state

@REM change main files name if needed due to conf.ini
:change_filename
...
echo.
AppsDataBase\m.exe %menuhover% "| Create account         [ Default  ]" "| Look at functions      [ Guest    ]" "| I already have account [ Error?   ]" "| Exit gConsolePassX     [ Why now? ]" "| Restore backup         [ *.backup ]"
if errorlevel == 5 goto restore_backup
if errorlevel == 4 goto exit_app_guest
if errorlevel == 3 goto conferror
if errorlevel == 2 goto as_guest
if errorlevel == 1 goto create_ac

@REM Create gConsolePassX account
:create_ac
cls
echo ==============================================================================
echo.                   gConsolePassX -Console Password Manager.
...
echo. This files are used by gConsolePassX to store all types of data.
...
echo. -----------------------------------------------------------------------------
echo.
echo. Selecting no will exit app without saving any setup progress.
AppsDataBase\m.exe %menuhover% "| Yes I agree      |" "| No I don't agree |"
...
@REM New users can look at all gConsolePassX's functions.
:as_guest
cls
echo ==============================================================================
echo.                   gConsolePassX -Console Password Manager.
...
echo. Select the Menu-options (you can use KEYBOARD or MOUSE)
echo.
AppsDataBase\m.exe %menuhover% "| [1] Change configuration [ Advanced ]" "| [2] Login to account     [ Default  ]" "| [3] Forgot password      [ Usual    ]" "| [4] Exit gConsolePassX   [ Why now? ]" "| [5] Restore backup       [ *.backup ]"
if errorlevel == 5 goto restore_backup
if errorlevel == 4 goto exit_app_guest
if errorlevel == 3 goto forgotpass_guest
if errorlevel == 2 goto login_task_guest
if errorlevel == 1 goto change_conf_guest
...
echo.
AppsDataBase\m.exe %menuhover% "| [1] Using gConsolePassX's console" "| [2] Using MS-Windows Notepad"
if errorlevel == 2 goto editconf_notepad_guest
if errorlevel == 1 goto editconf_nano_guest
...
echo. Select the Menu-options (you can use KEYBOARD or MOUSE)
echo.
AppsDataBase\m.exe %menuhover% "| [1] Change login information. |" "| [2] Check saved passwords.    |" "| [3] Edit password file.       |" "| [4] Add new passwords.        |" "| [5] Logout of account.        |" "| [6] Logout and exit.          |" "| [7] Check logs.               |" "| [8] Make backup.              |"
...
echo. How do you want to open this file?
echo.
AppsDataBase\m.exe %menuhover% "| [1] Using ConsolePassX's console |" "| [2] Using MS-Windows Notepad     |"
